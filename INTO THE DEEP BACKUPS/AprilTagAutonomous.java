...
public class AprilTagAutonomous extends LinearOpMode {
    ...
    */
    ...
    public void runOpMode() {
        currentGoalPos[0][0] = 22.7;
        currentGoalPos[1][0] = -52.5;
        initAprilTag();
        initializeHardware();
        ...
        int currentstep = 0;
        int poslen = 3;
        double[][] positionOrder = new double[poslen][3];
        positionOrder[0] = new double[] {10, -50, 0};
        positionOrder[1] = new double[] {10, -70, 0};
        ...
        while (opModeIsActive()) {
            ...
            if(moveToGoal() && currentstep < poslen) {
                currentstep += 1;
                changeTarget(currentGoalPos, positionOrder[currentstep]);
            }
            telemetry.addData("POSX", currentGoalPos[0][0]);
            telemetry.addData("GOALX", currentGoalPos[0][1]);
            telemetry.addData("POSY", currentGoalPos[0][0]);
            telemetry.addData("GOALY", currentGoalPos[0][1]);
    ...
    private boolean moveToGoal() {
        if (!anyBusy()) {
            ...
            if (diff != 0) {
                setDriveTicks((int)(diff * 100), "forward");
            ...
            if (diff != 0) {
                setDriveTicks((int)(diff * 45), "left");
                setDriveRunModes(DcMotor.RunMode.RUN_TO_POSITION);
    ...
    private double largeDiff(double n1, double n2) {
        if (Math.abs(Math.abs(n1) - Math.abs(n2)) > 3) {
            double diff = n1 - n2;
            return diff / Math.abs(diff);
        }
        return 0;
    }
    private void setDriveTicks(int ticks, String direction){
