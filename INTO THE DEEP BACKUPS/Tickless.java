...
@TeleOp
public class Tickless extends LinearOpMode {
    boolean claw1 = false;
    boolean claw2 = false;
    double rot_pos = 0.5;
    int vpos = 0;
    int hpos = 0;
    int FLpos, FRpos, BLpos, BRpos, ARMpos = 0; //Zeyad
    CRServo sub_peck;
    ...
    public void runOpMode(){
        ...
        frontRightMotor.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);//Zeyad
        frontLeftMotor.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);//Zeyad
        backRightMotor.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);//Zeyad
        backLeftMotor.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);//Zeyad
        ...
        while (opModeIsActive()) {
            ...
            if (gamepad1.right_bumper){
                ...
                ARMpos+=1;
                //manager.runTicks(armMotor, ARMpos, 1);
                /*if (!armMotor.isBusy()) {
                    armMotor.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
            ...
            if (gamepad1.a) {
                ...
                manager.runTicks(frontLeftMotor, -100, 1);
                manager.runTicks(frontRightMotor, -100, 1);
                manager.runTicks(backLeftMotor, -100, 1);
                manager.runTicks(backRightMotor, -100, 1);*/
                FLpos-=100; FRpos-=100; BLpos-=100; BRpos-=100;
                manager.runTicks(frontLeftMotor, FLpos, 1);
                manager.runTicks(frontRightMotor, FRpos, 1);
                manager.runTicks(backLeftMotor, BLpos, 1);
                manager.runTicks(backRightMotor, BRpos, 1);
            }
            if (gamepad1.b) {
