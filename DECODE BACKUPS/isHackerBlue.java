...
@TeleOp(name="IsHackerBlue")
public class isHackerBlue extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) { //NOTE: I HAVE SWAPPED THE STICK X's FOR A QUICK FIX TO A PROBLEM. FIX IT AND THEN SWITCH THEM BACK!!!
            ...
            if (yaw != null) { 
                ...
                if ((rightTrigger||(gamepad2.right_trigger>0))){
                    Movement(0, 0, yaw, 10000);
                    engaged = false;
            ...
            if (gamepad1.a && (fly_dir == 0) && !lastFlyToggle){
                fly_dir = 1;
            } else if (gamepad1.a && (fly_dir != 0) && !lastFlyToggle){
                fly_dir = 0;
            }
    ...
    private Double getTagYaw(int tagId) { 
        ...
        for (AprilTagDetection detection : detections) { 
            if (detection.id == tagId && detection.ftcPose != null) {
                ...
                yc = detection.ftcPose.y + 0.1537 * Math.sin(angle * Math.PI / 180) + 0.1537 * Math.cos(53.673 * Math.PI / 180 - angle * Math.PI / 180);
                xc = detection.ftcPose.x - 0.1537 * Math.sin(53.673 * Math.PI / 180 - angle * Math.PI / 180) - 0.1537 * Math.cos(angle * Math.PI / 180);
    ...
    // NOTE NOTE NOTE: TS IS NOT THE SAME AS IN THE AUTOS. IDK WHY WE EVER USED POWER, BUT WE USE VELOCITY NOW
    public void Movement(double x, double y, double rx, double power) {
        ...
        //set power
        frontLeftMotor.setVelocity(power);
        frontRightMotor.setVelocity(power);
        backLeftMotor.setVelocity(power);
        backRightMotor.setVelocity(power);
    }
