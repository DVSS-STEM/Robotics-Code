...
public class isHackerBlueBackup extends LinearOpMode {
    ...
    private Double getTagYaw(int tagId) { 
        ...
        for (AprilTagDetection detection : detections) { 
            if (detection.id == tagId && detection.ftcPose != null) {
                yc = detection.ftcPose.y;
                xc = detection.ftcPose.x - 0.05*(yc*yc);
                ...
                if (angle >= Math.atan(xc/yc) || angle <= Math.atan(xc/yc) ){
                    if (Math.atan(xc/yc) >= 0){
                        x = -0.05; //right
                    }
                    else{
                        x = 0.05; //left
