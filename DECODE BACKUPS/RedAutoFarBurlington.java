...
public class RedAutoFarBurlington extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            switch (stage) {
                ...
                case 4:
                    if(startTime.seconds() >= 1) {
                        intakeMotor.setVelocity(1500); // starts intaking to shoot and waits 13 seconds for it to fully unload (in next conditional)
                        stage += 1;
                        startTime.reset();
                    }
                    break;
                ...
                case 6:
                    if(startTime.seconds() >= 0){
                        Movement(0.8, 0, 0,0.8); // forward to pick up pieces
                        WaitForDrive();
                        stage += 1;
                        startTime.reset();
                    }
                ...
                case 8:
                    if(startTime.seconds() >= 0){
                        ...
                        Movement(-0.65, 0.8, 0, 0.8); // backright to get into shooting position CHANGE THIS
                        WaitForDrive();
                        stage += 1;
                        startTime.reset();
                    }
                ...
                case 12:
                    if(startTime.seconds() >= 6){
                        ...
                        rollServo.setPosition(0);
                        Movement(0.5, 0, 0,0.8); // leaves the zone CHANGE THIS
                        WaitForDrive();
                        stage += 1;
                        startTime.reset();
                    }
